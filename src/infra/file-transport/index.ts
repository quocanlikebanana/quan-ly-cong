...
export class FileTransport {
	...
	static async writeImagePreviewFile(fileContent: Buffer | Uint8Array | string, key: string, to: "s3" | "local" = "local") {
		const transport = this.getTransport(to);
		return transport.writeImagePreviewFile ? transport.writeImagePreviewFile(fileContent, key) : false;
	}

	static async readImagePreviewFile(key: string, from: "s3" | "local" = "local"): Promise<string> {
		const transport = this.getTransport(from);
		return transport.readImagePreviewFile ? transport.readImagePreviewFile(key) : '';
	}
